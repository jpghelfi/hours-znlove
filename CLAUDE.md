...
- **Per-project view** (`/project`, admins): one project, **one** period (a month, a week or a day — not a window of several), rendered as a plain person/hours table. `_period_range` in `web/app.py` owns all three granularities: it returns the date bounds, the label, the prev/now/next anchors and which native picker to show (`date` vs `month`), so adding a granularity is one branch there. `?start=` is the anchor — `_project_anchor` widens the month picker's `YYYY-MM` to the 1st before parsing. Reads go through `ops.project_entries`, which filters on the Project relation *in the Notion query* rather than scanning all entries in Python.
