...
- **One grid cell can hold several entries**: a person may log two different tasks on the same project and day, and the weekly grid sums them into one number. So `set_cell` never assumes a cell is one row — it groups the day's rows by normalized description: rows whose text matches are a genuine duplicate and fold into one, but rows with *different* text are different tasks and the write is **refused** (`{"ok": False, "code": "multi_entry", "entries": [...]}`) so nothing is silently archived. The week page answers that by opening the day editor (`/api/day`, `/api/entry/{add,update,delete}`), one line per entry; the `⋯` affordance on any cell opens the same editor, which is how a day gets *split* in the first place. `week_grid` rows carry `counts` alongside `cells` so the UI can badge multi-entry cells.
- **Entry ids from the browser are never trusted**: `_owned_entry` re-fetches the page and checks both that it lives in Time Entries and that its `Person` is the caller before any per-entry write.
- **`sorts` with `{"timestamp": "created_time"}` is silently ignored** by the 2025-09-03 data-source query (property sorts do work) — it returns unsorted results with no error, so order by `created_time` in Python instead. `created_time` is only minute-granular, so tie-break on id.
