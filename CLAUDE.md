...
- **People roster** (`list_people` in `web/notion_ops.py`): the people shown everywhere (assignments columns, schedule rows, dropdowns) come from the **People** database (curated in Notion: `Active` checkbox hides, row title renames, `Person` link is required). When `people_ds_id` isn't configured it falls back to listing all workspace members. New workspace members don't appear until `setup_people_db.py` is re-run or a row is added by hand.
...
- **Weekly grid & allocations**: cell edits are upserts keyed on (person, project, date/week); hours = 0 deletes the entry. Allocation writes are serialized with a lock (`_set_allocation_locked`) to avoid duplicate rows from concurrent upserts.
